...
_logger = logging.getLogger('cyPyUSB.backend.libusb0')
...
class _LibUSB(cyPyUSB.backend.IBackend):
    ...
    def get_configuration(self, dev_handle):
        bmRequestType = cyPyUSB.util.build_request_type(
                                cyPyUSB.util.CTRL_IN,
                                cyPyUSB.util.CTRL_TYPE_STANDARD,
                                cyPyUSB.util.CTRL_RECIPIENT_DEVICE
                            )
        buff = cyPyUSB.util.create_buffer(1)
    ...
    def iso_write(self, dev_handle, ep, intf, data, timeout):
        if not _has_iso_transfer():
            return cyPyUSB.backend.IBackend.iso_write(self, dev_handle, ep, intf, data, timeout)
        return self.__iso_transfer(dev_handle, ep, intf, data, timeout)

    @methodtrace(_logger)
    def iso_read(self, dev_handle, ep, intf, buff, timeout):
        if not _has_iso_transfer():
            return cyPyUSB.backend.IBackend.iso_read(self, dev_handle, ep, intf, buff, timeout)
