...
import sys
import logging
from cyPyUSB._debug import methodtrace
import cyPyUSB._interop as _interop
import cyPyUSB._objfinalizer as _objfinalizer
import errno
import math
from cyPyUSB.core import USBError
import cyPyUSB.libloader
...
_logger = logging.getLogger('cyPyUSB.backend.libusb1')
...
def _load_library(find_library=None):
    ...
    # object libcyPyUSB.so, so if we found libusb library name, we must assure
    ...
    return cyPyUSB.libloader.load_locate_library(
...
def _setup_prototypes(lib):
    ...
    def libusb_set_iso_packet_lengths(transfer_p, length):
        r"""This function is inline in the libcyPyUSB.h file, so we must implement
            it.
...
class _Device(_objfinalizer.AutoFinalizedObject):
    ...
    def _finalize_object(self):
        return # Added Return. Seems to Fix: OSError: exception: access violation writing 0x00000014
...
class _IsoTransferHandler(_objfinalizer.AutoFinalizedObject):
    ...
    def submit(self, ctx = None):
        ...
        if status != LIBUSB_TRANSFER_COMPLETED:
            raise cyPyUSB.USBError(_str_transfer_error[status],
                               status,
...
class _LibUSB(cyPyUSB.backend.IBackend):
    @methodtrace(_logger)
    def __init__(self, lib):
        cyPyUSB.backend.IBackend.__init__(self)
        self.lib = lib
        self.ctx = c_void_p()
...
def get_backend(find_library=None):
    ...
    try:
        ...
        return _LibUSB(_lib)
    except cyPyUSB.libloader.LibraryException:
